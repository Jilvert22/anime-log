    ...
    >
      ...
      <div className="flex items-center gap-4 text-sm">
        <span className="text-gray-600 dark:text-gray-400">
          <span className="font-bold" style={{ color: '#764ba2' }}>
            {stats.total}
          </span>{' '}
          作品
        </span>
        ...
        <span className="text-gray-600 dark:text-gray-400">
          神作{' '}
          <span className="font-bold" style={{ color: '#e879d4' }}>
            {stats.godTier}
          </span>
        </span>
      </div>
...
export const SeasonHeader = memo(function SeasonHeader({
  ...
  return (
    <div
      className={`w-full ml-4 ${isEmpty ? 'border border-dashed border-gray-300 dark:border-gray-600 rounded-lg' : ''}`}
    >
      <div
        className={`flex items-center justify-between py-2 px-3 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors ${
          isEmpty ? '' : ''
        }`}
      >
        <button onClick={onToggle} className="flex-1 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-gray-400 text-sm">{isExpanded ? '▼' : '▶'}</span>
            <span
              className={`font-medium ${isEmpty ? 'text-gray-400 dark:text-gray-500' : 'text-gray-700 dark:text-gray-300'}`}
            >
            ...
            {isEmpty ? (
              ...
              <>
                <span className="text-gray-500 dark:text-gray-400">
                  <span className="font-medium" style={{ color: '#764ba2' }}>
                    {stats.total}
                  </span>{' '}
                  作品
                </span>
                ...
                {stats.godTier > 0 && (
                  <span className="text-gray-500 dark:text-gray-400">
                    神作{' '}
                    <span className="font-medium" style={{ color: '#e879d4' }}>
                      {stats.godTier}
                    </span>
                  </span>
                )}
