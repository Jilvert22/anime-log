...
import { UserContentBoundary, useModeration } from '../../contexts/ModerationContext';
...
import type { Anime as DomainAnime } from '../../types';
import { useFeedback } from '../../contexts/FeedbackContext';

type Anime = Pick<DomainAnime, 'id' | 'title' | 'image' | 'rating'>;
...
export default function ProfilePageClient({
  ...
  const { user, isLoading: authLoading } = useModeration();
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        const [records, counts, following] = prof
          ? await Promise.all([
              getPublicAnimes(prof.id),
              getFollowCounts(prof.id),
              viewerId && viewerId !== prof.id ? isFollowing(prof.id) : false,
            ])
          : [[], { following: 0, followers: 0 }, false];
        if (!active) return;
        setProfile(prof);
        setAnimes(records as Anime[]);
  ...
  }, [username, viewerId, authLoading, revision]);
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-800 dark:text-white mb-4">
            {loadError ? 'プロフィールを読み込めませんでした' : 'プロフィールが見つかりません'}
          </h1>
          {loadError && (
            <button className="underline p-3" onClick={() => setRevision((value) => value + 1)}>
              再試行
            </button>
          )}
          <button
  ...
  return (
    <UserContentBoundary userId={profile.id}>
