...
- `npm run lint` — ESLint。`--max-warnings 136` の**警告バジェット**(後述)
...
- `Anime.id`(`AnimeId = string | number`)は**二重実態**: **ログイン時**は Supabase `animes.id`(uuid 型)の
  **UUID 文字列**、**未ログイン時**は localStorage の**合成 number**(`Math.max(...ids)+1`)。number id は DB に存在し得ない。
  作品の同一性は **`anilist_id`**(ドメイン型では `anilistId`)で判断する。数値演算(`Math.max`/比較)は `typeof id === 'number'` で必ずガードする。
- 旧慣習の `id < 1_000_000`(AniList ID 判定)は廃止。AniList 詳細取得は `anilistId` の有無で判定する。
  `id > 1_000_000`(合成 number = ローカル行の DB 削除スキップ)判定は `AnimeDetailModal` の削除ガードにのみ残存。
...
- **警告バジェット(ラチェット)**: `--max-warnings 136`。**警告を増やす変更は CI が落ちる**。
