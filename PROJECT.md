...
このドキュメントは、ClaudeなどのAIアシスタントにプロジェクト情報を渡すために作成されました。

### Claudeにプロジェクト情報を渡す方法

#### Cursor内のClaudeに渡す場合

1. **ファイルをドラッグ&ドロップ**
   - Cursorのチャット画面に `PROJECT.md` ファイルをドラッグ&ドロップ

2. **@記号でファイルを指定**
   - チャットで `@PROJECT.md` と入力

3. **ファイルパスを指定**
   - チャットで「`PROJECT.md`を参照してください」と伝える

#### 別のClaude環境（Webアプリなど）に渡す場合

**方法1: ファイルをコピーして貼り付け（最も簡単）**

1. Cursorで `PROJECT.md` を開く
2. ファイル全体を選択（Cmd+A / Ctrl+A）
3. コピー（Cmd+C / Ctrl+C）
4. ClaudeのWebアプリなどのチャット画面に貼り付け（Cmd+V / Ctrl+V）
5. 必要に応じて「このプロジェクトドキュメントを参照してください」と伝える

**方法2: ファイルをダウンロードして添付**

1. Cursorで `PROJECT.md` を右クリック
2. 「Reveal in Finder」（Mac）または「エクスプローラーで表示」（Windows）を選択
3. ファイルをClaudeのWebアプリなどにドラッグ&ドロップ

**方法3: ファイルパスを共有**

- ファイルの場所: `/Users/riku/Projects/anime-log/PROJECT.md`
- このパスをClaudeに伝える（ただし、Webアプリでは直接アクセスできないため、方法1または2を推奨）

**推奨**: 方法1（コピー&ペースト）が最も簡単で確実です。

### このドキュメントの目的

毎回同じ情報を説明する手間を省き、効率的にプロジェクトについて相談できるようにするためです。
