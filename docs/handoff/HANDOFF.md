...
- **分析フェーズ**:
  ...
  2. **GSC の現状データ**を CLI で取得する。gcloud 導入済み(`riku@raside.com`)だが、(a) トークン失効で
     `gcloud auth print-access-token` が非対話では再認証不可、(b) Search Console API には `webmasters.readonly`
     スコープが必要(標準ログインに含まれない)。→ **ユーザーが一度だけ対話ログイン**が要る:
     `gcloud auth login --enable-gdrive-access=false --update-adc --scopes=openid,https://www.googleapis.com/auth/webmasters.readonly,https://www.googleapis.com/auth/cloud-platform`
     ＋ その account が animelog.jp の GSC プロパティにアクセス権を持つこと。以後は
     `curl -H "Authorization: Bearer $(gcloud auth print-access-token)" https://www.googleapis.com/webmasters/v3/sites/<siteUrl>/searchAnalytics/query -d @query.json` で
     クエリ/表示/クリック/掲載順位を AI が取得できる。まずセットアップ可否をユーザーに確認。
...
- **アナリティクス連携（ユーザー希望）**:
  - **Vercel Analytics は既に連携済み**（`@vercel/analytics`、`app/layout.tsx` に `<Analytics />`、`track()` 使用中）→ 新規設定不要。
    余力で **Speed Insights**（`@vercel/speed-insights`、未導入）を足すと Core Web Vitals をダッシュボードで追える。
  - **GA4 は未連携** → 新規に入れる。`@next/third-parties/google` の `<GoogleAnalytics gaId="G-XXXX" />` が最短。
    **要ユーザー入力**: GA4 の Measurement ID(G-XXXX)。**要判断**: GA4 は Cookie を使うため、現行プライバシーポリシー
    （`app/privacy/page.tsx` は「Cookie を使わない Vercel Analytics」を明記）の更新と同意まわりの検討が必要。
