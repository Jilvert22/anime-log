...
### このセッション（2026-07-22）— 重複URL対策 2 PR ＋ GSC ドメインプロパティ化（すべて本番反映・実測済み）

- **#66 / #67 重複URLを実体で塞ぐ**: `proxy.ts`（Next 16 のミドルウェア規約。既存の Supabase 認証更新に統合）冒頭で、**本番デプロイ（`VERCEL_ENV==='production'`）かつ apex 以外のホストを apex（`animelog.jp`）へ 308**。#66 は `*.vercel.app` 限定、#67 で `host !== CANONICAL_HOST`（www 含む apex 以外すべて）に一般化。本番実測: `anime-log-rho.vercel.app` / `www.animelog.jp` → 308→apex（パス＋クエリ保持）、apex=200 でループ無し。Codex レビュー2回とも通過（Host 正規化・fail-closed 注意コメントを反映）。対象: [proxy.ts](../../proxy.ts)。
- **GSC ドメインプロパティ化（C）**: 従来 URL プレフィックス型 `https://animelog.jp/`（apex 1オリジンのみ）→ **ドメインプロパティ `sc-domain:animelog.jp` を新規作成・DNS TXT 認証で検証済み**（apex+www+http/https+全サブドメインを一元カバー）。TXT は **Vercel CLI で追加**（`vercel dns add animelog.jp @ TXT "google-site-verification=..."`。animelog.jp は Vercel 管理 NS）。**今後の GSC データ取得は `sc-domain:animelog.jp` を使う**。サイトマップ `https://animelog.jp/sitemap.xml` は登録済み（5ページ・成功）。詳細は記憶 `anime-log-seo-backlog`。

### 前セッション（2026-07-21・P2 完遂）— 7 PR、すべて CI 緑でマージ＋本番検証済み
...
コード側は **なし**（作業ツリークリーン・main 全マージ・全ゲート緑）。下記 P1 はフィールドデータ待ちの検証タスク。

**ユーザー側の手動フォロー（急ぎでない・コード作業なし）**:
- **1日後**: GSC ドメインプロパティにデータ（過去分バックフィル含む）が乗ったのを確認 → 旧 URL プレフィックス型 `https://animelog.jp/` を 設定→プロパティ削除で一本化（過去データはドメイン型が最大16ヶ月バックフィルするため消えない）。
- **（任意）GA4↔GSC リンク**: GA4 管理→サービス間のリンク→Search Console でドメインプロパティ `animelog.jp` を選ぶと GA4 内で検索クエリが見られる。必須ではない。
- **aquarium-map.com（別プロジェクト・気が向いた時）**: animelog と同型の www 重複（www が 200・Vercel・canonical は apex）を診断済み。同じ3点セット（GSC ドメインプロパティ／www→apex／vercel.app→apex）が適用可能だが、**DNS がお名前.com（`dnsv.jp`）＋リダイレクトは別リポジトリ作業**で割高。実利は「整え・保険」で流入増ではない＝急ぎ不要。

## 直近の決定と理由

- **リダイレクトは apex 以外すべて（fail-closed 設計）**: `host !== CANONICAL_HOST` に一般化。将来 Vercel production に別ドメイン（短縮リンク/キャンペーン用等）を追加すると自動で apex へ 308 される（`proxy.ts` にコメント済）。残したいドメインは `CANONICAL_HOST` 側で許可する。
- **vercel.app/www は canonical で既に緩和済み＝今回は「保険」**: 実際 vercel.app は Google 未インデックス（`site:` 実測ゼロ）だった。順位/流入は増えず、実利は「共有時 URL 統一・分析のホスト分裂防止・実体重複の除去」。GSC ドメインプロパティも同様に「整え」で、見えるページ単位の中身は URL プレフィックス型と変わらない（増えるのは www/http/サブドメイン枠のみ）。
...
- **Next.js 16 のミドルウェアは `proxy.ts`（`middleware.ts` は非推奨リネーム）**: 本リポは既に `proxy.ts`（Supabase 認証更新）を持つ。新規 `middleware.ts` を作ると**両ファイル併存でサーバー起動が「Both middleware file and proxy file are detected」で落ち、E2E/Vercel ビルドが失敗**する。**ローカルの type-check/lint/vitest は通る**（dev/build のサーバー起動時にしか出ない）＝ユニットだけでは検知不可。ミドルウェア相当は必ず既存 `proxy.ts` に足す。
